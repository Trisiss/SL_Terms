    ...
    private fun mayRequestContacts(): Boolean {
        ...
        if (shouldShowRequestPermissionRationale(permission.READ_CONTACTS)) {
            Snackbar.make(mEmailView!!, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
                    .setAction(android.R.string.ok) { requestPermissions(arrayOf(permission.READ_CONTACTS), REQUEST_READ_CONTACTS) }
        } else {
...
// perform the user login attempt.
    ...
    private fun showProgress(show: Boolean) { // On Honeycomb MR2 we have the ViewPropertyAnimator APIs, which allow
...
// and hide the relevant UI components.
    ...
    override fun onCreateLoader(i: Int, bundle: Bundle): Loader<Cursor> {
        return CursorLoader(this,  // Retrieve data rows for the device user's 'profile' contact.
                Uri.withAppendedPath(ContactsContract.Profile.CONTENT_URI,
                        ContactsContract.Contacts.Data.CONTENT_DIRECTORY), ProfileQuery.PROJECTION,  // Select only email addresses.
...
// a primary email address if the user hasn't specified one.
    ...
    private fun addEmailsToAutoComplete(emailAddressCollection: List<String>) { //Create adapter to tell the AutoCompleteTextView what to show in its dropdown list.
    ...
    inner class UserLoginTask internal constructor(private val mEmail: String, private val mPassword: String) : AsyncTask<Void?, Void?, Boolean>() {
        protected override fun doInBackground(vararg p0: Void?): Boolean? { // TODO: attempt authentication against a network service.
            ...
            for (credential in DUMMY_CREDENTIALS) {
                ...
                if (pieces[0] == mEmail) { // Account exists, return true if the password matches.
